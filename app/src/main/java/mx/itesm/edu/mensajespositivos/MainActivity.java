package mx.itesm.edu.mensajespositivos;
...
public class MainActivity extends AppCompatActivity {
    ...
    public static int last_i = 0;
    private EditText editText;
    private static final String TAG = "POSITIVE";
    public static ArrayList<String> resultMessagesList;
    public static int resultMessagesListIndex = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rnd = new Random();
        context = this;
        resultMessagesList = new ArrayList<String>();

        this.editText = (EditText)findViewById(R.id.editText);

        Toast.makeText(this, "HARD CODED SEXY TEXT", Toast.LENGTH_LONG).show();
        Log.i(TAG, "Ok");
        ...
        int i = rnd.nextInt(messages.length);
        MainActivity.last_i = i;
        TextView messageTxtV = (TextView)findViewById(R.id.messageTxtV);
        messageTxtV.setText(messages[i].replace(".-", ".\n- "));
        MainActivity.resultMessagesList.add(messages[i].replace(".-", ".\n- "));
        MainActivity.resultMessagesListIndex = 0;
    }

    public void NextBtn_OnClick(View v) {
        int i = 0;
        do {
            i = rnd.nextInt(messages.length);
        } while (i == MainActivity.last_i);
        MainActivity.last_i = i;

        TextView messageTxtV = (TextView)findViewById(R.id.messageTxtV);
        messageTxtV.setText(messages[i].replace(".-", ".\n- "));
        MainActivity.resultMessagesList.add(messages[i].replace(".-", ".\n- "));
        MainActivity.resultMessagesListIndex = MainActivity.resultMessagesList.size() - 1;
    }

    public void DayBtn_OnClick(View v) {
        if (MainActivity.mode == 2) {
            ConstraintLayout layout = (ConstraintLayout)findViewById(R.id.parent);
    ...
    public void NightBtn_OnClick(View v) {
        ...
        // layout.setBackground(ContextCompat.getDrawable(MainActivity.context, R.drawable.night));
    }

    public void ForwardBtn_OnClick(View v) {
        int index = MainActivity.resultMessagesListIndex + 1;
        if (index > MainActivity.resultMessagesList.size() - 1) {
            index = 0;
        }

        TextView messageTxtV = (TextView)findViewById(R.id.messageTxtV);
        messageTxtV.setText(MainActivity.resultMessagesList.get(index));
        MainActivity.resultMessagesListIndex = index;
    }

    public void BackBtn_OnClick(View v) {
        int index = MainActivity.resultMessagesListIndex - 1;
        if (index < 0) {
            index = MainActivity.resultMessagesList.size() - 1;
        }
    ...
    public void SearchBtn_OnClick(View v) {
        try {
            ...
            int search_index = Integer.parseInt(input);
            if (search_index >= 1 && search_index <= MainActivity.resultMessagesList.size()) {
                TextView messageTxtV = (TextView) findViewById(R.id.messageTxtV);
                messageTxtV.setText(MainActivity.resultMessagesList.get(search_index - 1));
                MainActivity.resultMessagesListIndex = search_index - 1;
            } else {
                Toast.makeText(MainActivity.context, "Índice fuera de rango.", Toast.LENGTH_LONG).show();
            }
        }
        catch (Exception e) {
            Toast.makeText(MainActivity.context, "Input no válido.", Toast.LENGTH_LONG).show();
        }
    }
}
